        ...
        const _posts = Object.entries(posts
            ...
            .map((_post, key) => {
                ...
                __post.push(..._post[1].map((__post) => {
                    return `
                        <div><span style="margin-right:10px">${(new Date (__post.date)).toISOString()}</span><a href="http://localhost:${_envConfig.BLOG_API_PORT}/${_envConfig.API_PREFIX}/${EBlogRouts.info}/${EBlogRouts.posts}/${EBlogRouts.one}/${__post.id}" target="_blank">${__post.type}</a></div>
                    `
                }))
                __post.push(...[
                    lineBreak,
